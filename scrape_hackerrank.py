...
parser.add_argument(
    "-username", "-u", required=True, type=str, help="HackerRank username"
)
parser.add_argument(
    "-password", "-p", required=True, type=str, help="HackerRank password"
)
parser.add_argument(
    "-regenerate_cache",
    "-r",
...
parser.add_argument(
    ...
    default=300,
    type=int,
    help="Number of solution links to load per request. 500 is the highest I've tried that worked. ~1400 timed out.",
)
parser.add_argument(
    "-timeout",
    "-t",
    default=10,
    type=int,
    help="Number of seconds to wait between requests. I got banned briefly using someone else's tool. I'd err high.",
)
parser.add_argument(
    "-useragent",
    "-a",
    type=str,
    help="Useragent, you might run into a json error if it's out of date.",
...
if not logon_json["status"]:
    print(logon_json["errors"])
...
problem_list_path = os.path.join(
    cache_path, "problem_list"
)  # Generate cache for the raw list of submissions
if not os.path.isdir(problem_list_path):
    os.mkdir(problem_list_path)
    total_results = session.get(submissions_url.format(0, 0), headers=headers).json()[
        "total"
    ]  # retrieving total results for pagination
    ...
    for problem in problems_page:
        if problem["challenge_id"] not in problems_dict:
            problems_dict[problem["challenge_id"]] = {
                "id": problem["id"],
                "slug": problem["challenge"]["slug"],
            }
print("Number of unique problems:", len(problems_dict))

problem_data_path = os.path.join(
    cache_path, "problem_data"
)  # Generate cache for the raw problem data
if not os.path.isdir(problem_data_path):
    os.mkdir(problem_data_path)
for i, problem in enumerate(problems_dict.values()):
    problem_path = os.path.join(problem_data_path, str(problem["id"]))
    if not os.path.exists(problem_path):
        response = session.get(
            challenge_url.format(problem["slug"], problem["id"]), headers=headers
        )
...
for problem in os.listdir(problem_data_path):
    ...
    if problem_json["status"] == "Accepted":
        problems_dict[problem_json["id"]] = {
            ...
            for item in ["language", "code", "name", "slug", "status", "display_score"]
        }
